...
/*
  lcd_intout - Prints an integer onto the LCD screen
*/
void lcd_intout(int n)
{
  char out[15];
  snprintf(out, 15, "%d", n);
  lcd_writecommand(1);
...
/*
  lcd_intout - Prints an integer with specified coordinate on LCD screen
*/
void lcd_intout_coord(unsigned char row, unsigned char col, int n)
{
  char out[15];
  snprintf(out, 15, "%d", n);
  // lcd_writecommand(1);
...
/*
  lcd_numout - Converts decimal to hex and prints out readable
  numbers at the current cursor position
*/
void lcd_numout(int count)
{
...
void lcd_writecommand(unsigned char cmd)
{
  ...
  unsigned char topFour = cmd & 0xf0;
  lcd_writenibble(topFour);

  /* Call lcd_writenibble to send LOWER four bits of "cmd" argument */
  unsigned char btmFour = cmd & 0x0f;
  lcd_writenibble((btmFour << 4) & 0xf0);
